  ...
  <div class="user-list">
      ...
      <div class="dialog-body">
        <div class="form-row">
          <div class="form-section">
            ...
            <el-input v-model="addName" :placeholder="t('usermng.namePlaceHolder')" class="custom-input" />
          </div>
          <div class="form-section">
            <label class="form-label">{{ t('usermng.email') }}<span style="color: red">*</span></label>
            <el-input v-model="addEmail" :placeholder="t('usermng.emailPlaceHolder')" class="custom-input" />
          </div>
        </div>
        <div class="form-row">
          <div class="form-section">
            <label class="form-label">{{ t('usermng.password') }}<span style="color: red">*</span></label>
            <el-input v-model="addPassword" :placeholder="t('usermng.passwordPlaceHolder')" class="custom-input" />
          </div>
          <div class="form-section">
            <label class="form-label">{{ t('usermng.permission') }}<span style="color: red">*</span></label>
            <el-select class="custom-input" v-model="llm_name">
    ...
    <div class="top-section">
      <el-form label-position="left" label-width="auto" :inline="true" size="large">
        <el-form-item>
          <el-input v-model="usernameFilter" style="width: 330px;height: 40px;" :placeholder="t('usermng.inputTip')"
            @input="filterTable">
            <template #suffix>
        ...
        <el-form-item :label="t('usermng.addtime')">
          <el-date-picker v-model="dateRange" style="width: 370px;height: 40px;" type="datetimerange"
            ...
            format="YYYY-MM-DD HH:mm:ss" class="date-picker" @change="filterTable" :prefix-icon="Calendar" />
        </el-form-item>
      </el-form>
    </div>
...
import uploadIcon from '@/assets/icon-upload.svg?url';
import add2Icon from '@/assets/icon-add2.svg?url';
...
.user-list {
  ...
  .custom-dialog {
    ...
    .dialog-body {
      padding: 0 40px; // 为弹窗内容区增加水平内边距
    }

    // 新增：用于表单行的容器
    .form-row {
      display: flex;
      gap: 30px; // 设置同⾏元素之间的间距
      margin-bottom: 30px; // 设置行之间的垂直间距

      &:last-child {
        margin-bottom: 10px; // 调整最后一行与按钮的距离
      }
    }
