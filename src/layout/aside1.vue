    ...
    <div class="aside-left-nav">
      ...
      <div class="buttom-func">
        <div class="nav-button"  @click="goPage('userManagement')"  :class="{ active: aaaHovering }" @mouseenter="aaaHovering = true"
          ...
          <img src="@/assets/icon-usermanagement.jpg" alt="User Management" class="nav-menu-img-icon" />
        ...
        <div class="nav-button" @click="goPage('knowledgebaseManagement')"  :class="{ active: bbbHovering }" @mouseenter="bbbHovering = true"
          ...
          <img src="@/assets/icon-knowledge.jpg" alt="Knowledge Base" class="nav-menu-img-icon" />
          <span>{{ t('knowledgebaseManagement') }}</span>
        </div>
...
.aside {
  ...
  .buttom-func {
    ...
    .nav-button {
      ...
      .nav-menu-img-icon {
        width: 18px;  // 你可以根据你的图标实际显示效果调整这个值
        height: 18px; // 建议和宽度保持一致，或者根据图标比例调整
        margin-right: 8px; // 图片和文字之间的间距
        object-fit: contain; // 确保图片内容完整显示，不变形
        // vertical-align: middle; // 如果对齐有问题可以尝试，但 flex 布局下 align-items 通常足够
      }
